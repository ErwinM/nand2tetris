...
class Symboll
  ...
  def push
    return if Symboll.find(self.name)
    if self.type == "l_command" then
      self.address = $line_nr-1 # -1 because mem base is 0
    else
      self.address = $mem_pointer
      $mem_pointer += 1
    end
  ...
  file.readlines.each do |line|
    ...
    if instruction
      if instruction.type!="l_command" then
        instruction.push
        puts "Pushing command: #{instruction}"
      end
      unless instruction.command.is_num? || instruction.type == "c_command" then
        ...
        symbol.push
        puts "Pushing symbol: #{instruction}"
      end
    end
   end
end
  ...
  $command_table.each do |line, instr|
    ...
    if instr.type == "c_command" then
      ...
      if instr.is_symbol then
        puts "Cannot find symbol in table" if !Symboll.find(instr.command)
        addr = Symboll.find(instr.command).address
      else
